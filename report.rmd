...
libraries("minfi", "RColorBrewer", "ggplot2", "gap", "gplots", "tidyr", "reshape2", "magrittr", "dplyr", "tibble", "future.apply", "data.table")
```
...
intensities <- data.frame(getBeta(MSet.raw)) %>%
        rownames_to_column(var = "ID") %>%
        data.table() %>%
        pivot_longer(-ID, names_to = "Slide", values_to = "RawBeta")
M_values <- data.frame(getM(MSet.raw)) %>%
...
intensities["Group"] <- ifelse(intensities$Slide %in% slide_DS, "DS", "WT")
```
